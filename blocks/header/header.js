...
// Global variables for search panel access
let searchPanel;
let toggleSearchFn;
...
function closeOnEscape(e) {
  if (e.code === 'Escape') {
    ...
    if (
      searchPanel &&
      searchPanel.classList.contains('nav-tools-panel--show')
    ) {
...
export default async function decorate(block) {
  ...
  searchPanel = navTools.querySelector('.nav-search-panel');
  ...
  // Define toggleSearch and assign to global variable
  toggleSearchFn = async function toggleSearch(state) {
    const show =
    ...
    if (show) {
      // Import search configuration from searchbar.js module
      ...
      provider.render(SearchBarInput, {
        ...
        routeSearch: (searchQuery) => {
          const url = `${rootLink('/apparel')}?q=${encodeURIComponent(
            searchQuery,
          )}`;
      ...
      provider.render(SearchBarResults, { storeDetails })(searchResult);

      // Wait for the component to render and then focus the input
      setTimeout(() => {
